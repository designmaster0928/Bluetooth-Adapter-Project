...
#define INT 7
#define BAT_IN 19
#define STBY 2  // when the battery charign is completed, STBY is pulled low, otherwise, it will be High impedance.
#define CHRG 1  // when charger is charing the battery, CHRG is pulled low, otherwise, it will be High impedance.

#define CYCLE_MODE_TIME 5000            // 5000 milliseconds
#define BATTERY_SHOW_TIME 500           // 500 milliseconds
#define BUTTON_TIME 100                  // 50 milliseconds
#define BLUETOOTH_CONNECTION_TIME 1000  // 1000 miliseconds

#define POWEROFF_COUNT 20  // buttonCallbackTime = 50ms; set powerOfftime as 2s
...
ezButton KEY(5);      // create ezButton object that attach to pin 5;
...
const char* SppName = "KLEIN_PTT_SPP";
...
//Tasks
Task CycleModeTask(BLUETOOTH_CONNECTION_TIME, TASK_FOREVER, &cycleModesCallback);
...
void cycleModesCallback() {
  ...
  BleSpp = (bt.BleStatus() << 1) | bt.SppStatus();

  if (digitalRead(STBY) && !digitalRead(CHRG)) {
    // Charging battery
    rgbLed(1, 0, 0);  // RED color
  } else if (digitalRead(CHRG) && !digitalRead(STBY)) {
  ...
  if (batLevel < 313) {                   // <10% : 3.12V is 10% of max 4.2V
    ...
    if (BleSpp > 0) CycleModeTask.setInterval(CYCLE_MODE_TIME);
    else CycleModeTask.setInterval(BLUETOOTH_CONNECTION_TIME);
  }
}
void buttonCallback() {
  ...
  // BleSpp = bt.BleSppStatus();
  if (BleSpp > 0) {
    if (PTT_IN.isPressed()) {
      if (swMode) {
        if (BleSpp == 1)  // Only Classic bluetooth is connected
        {
          bt.sendData("+PTTS=P");
        } else if (BleSpp == 2)  // Only BLE is connected
        {
          bt.sendData("1");
        } else  //  Both BLE and Both BLE and Classic bluetooth are connected
        {
          bt.sendData("+PTTS=P");
          delay(20);
          bt.sendData("1");
...
void setup() {
  ...
  delay(100);
  //bt.bleOnOff(1);  // turn on oth
  //  bt.ServiceUUID(ServiceUUID);
  //  bt.CharacteristicUUID(CharacteristicUUID);
  //  bt.ServiceUUID("F000");
  //  bt.CharacteristicUUID("F002");
  bt.setVOl(30);  // set Volume 30(max)
  delay(100);
  bt.switchFunction(bt.eBluetooth);
  delay(100);
  bt.QueryCMD("C100");  // Turn off the AD button---pin PA5 of the chip doesn't need to pull up 22k.
  delay(100);
