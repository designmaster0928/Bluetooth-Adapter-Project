...
public:
  ...
  */
  ...
  bool SppName(const char* name);      // default : "BT201‑AUDIO
  bool BleName(const char* name);      // default : "BT201‑BLE
  bool btPassword(const char* pass);  // default 0000
  bool btMAC(const char* addr);
  bool playPause();
  bool bleOnOff(uint8_t vol);
  bool edrOnOff(uint8_t vol);
  void sendData(String data);
  bool SppStatus();
  bool BleStatus();
  bool ServiceUUID(const char* name);
  bool CharacteristicUUID(const char* name);
  bool QueryCMD(const char* cmd);
  String QueryName();
  uint8_t BleSppStatus();
protected:
